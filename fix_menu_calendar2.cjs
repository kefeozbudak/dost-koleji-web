...
const regex = /<ul className="space-y-1 md:space-y-2 whitespace-normal md:whitespace-pre-line">\s*\{\(Array\.isArray\(day\.meals\)(?:.|\n)*?<\/ul>/g;

const newMeals = `                            {day.breakfast && (
                              <div className="mb-2 whitespace-normal md:whitespace-pre-line">
                                ...
                                <ul className="space-y-0.5 md:space-y-1 whitespace-normal md:whitespace-pre-line">
                                  {day.breakfast.split(/[,\\n]+/).map((s) => s.trim()).filter(Boolean).map((meal, mIndex) => (
...
if (content.match(regex)) {
  content = content.replace(regex, newMeals);
  fs.writeFileSync('src/components/PageBlocks.tsx', content);
  console.log("Updated both instances with regex");
} else {
