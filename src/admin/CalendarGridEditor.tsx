      ...
      <div className="grid grid-cols-7 gap-1 mb-4">
        ...
        {Array.from({ length: daysInMonth }).map((_, i) => {
          ...
          const hasData = currentArray.some((d: any) => {
            ...
            return matchesDate && (d.meals?.length > 0 || d.events?.length > 0 || d.eventTitle || d.breakfast?.length > 0 || d.lunch?.length > 0 || d.snack?.length > 0);
          });
          return (
            <button
              key={dayNum}
          ...
          {block.type === 'menu_calendar' && (
            <>
              <div className="flex flex-col gap-1">
                <label className="text-[10px] font-bold text-slate-500 uppercase">Sabah Kahvaltısı</label>
                <textarea
                  value={activeItem.breakfast !== undefined ? activeItem.breakfast : (activeItem.meals || activeItem.events || "")}
                  onChange={(e) => updateActiveItem('breakfast', e.target.value)}
                  className="w-full px-2 py-2 text-xs border border-slate-300 rounded bg-white min-h-[60px]"
                  placeholder="Örn: Haşlanmış Yumurta, Peynir, Zeytin"
                />
              </div>
              <div className="flex flex-col gap-1">
                <label className="text-[10px] font-bold text-slate-500 uppercase">Öğle Yemeği</label>
                <textarea
                  value={activeItem.lunch || ""}
                  onChange={(e) => updateActiveItem('lunch', e.target.value)}
                  className="w-full px-2 py-2 text-xs border border-slate-300 rounded bg-white min-h-[60px]"
                  placeholder="Örn: Mercimek Çorbası, İzmir Köfte, Pilav"
                />
              </div>
              <div className="flex flex-col gap-1">
                <label className="text-[10px] font-bold text-slate-500 uppercase">İkindi Beslenmesi</label>
                <textarea
                  value={activeItem.snack || ""}
                  onChange={(e) => updateActiveItem('snack', e.target.value)}
                  className="w-full px-2 py-2 text-xs border border-slate-300 rounded bg-white min-h-[60px]"
                  placeholder="Örn: Meyve, Kuruyemiş, Süt"
                />
              </div>
              <div className="flex flex-col gap-1">
                <label className="text-[10px] font-bold text-slate-500 uppercase">Toplam Kalori</label>
                <input
                  type="text"
